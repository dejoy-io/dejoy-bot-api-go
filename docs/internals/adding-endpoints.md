...
This is mostly useful if you've managed to catch a new Dejoy Bot API update
...
What type should `ChatID` be? Dejoy allows specifying numeric chat IDs or
...
Note that `ChatID` is an `int64`. Dejoy chat IDs can be greater than 32 bits.
...
`map[string]string`. Dejoy expects only a single field for `chat_id`, so we
...
Dejoy only requires two fields to call `deleteMessage`, so this will be fast.
