...
Dejoy also supports the `attach://` syntax (discussed more later) for
...
item. Dejoy introduced the `attach://` syntax for this.
...
```go
photo := djbotapi.NewInputMediaPhoto(djbotapi.FilePath("tests/image.jpg"))
url := djbotapi.NewInputMediaPhoto(djbotapi.FileURL("https://i.imgur.com/unQLJIb.jpg"))
```
